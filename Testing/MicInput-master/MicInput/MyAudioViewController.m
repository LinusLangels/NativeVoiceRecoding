...
- (IBAction)audioSwitch:(id)sender {
    if (!audioSwitch.on) {
        ...
        bool status = false;
        status = [audioProcessor stop];
        if(true == status)
            NSLog(@"Recording succesful: true");
        else
            NSLog(@"Recording succesful: false");
        ...
        NSString *filename = [audioProcessor start:0 withfilepath:documentsDirectory withGuid:@"1234-3214-3245-1234" withAlgoNbr:1];
        [self showLabelWithText:@"AudioUnit running"];
        NSLog(@"File name and path is %@",filename);
    }
